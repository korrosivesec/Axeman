        ...
        for entry in result_info['entries']:
            ...
            lines.append(
                ",".join([
                    ...
                    str(int(cert_data['leaf_cert']['not_before'])),
                    str(int(cert_data['leaf_cert']['not_after']))
                ]) + "\n"
            )

        print("[{}] Finished processing, writing Parquet...".format(os.getpid()))

        df = pv.read_csv(BytesIO("".join(lines).encode('utf-8')), read_options=pv.ReadOptions(column_names=['ctl_log', 'index', 'chain_hash', 'cert_data', 'domains', 'not_before', 'not_after']))
